...
type AudioStreamOptions = {
  text: string;
  signal?: AbortSignal;
  onChunk?: (audioBase64: string, alignment: AlignmentData | null, normalizedAlignment: AlignmentData | null) => void;
};
...
export async function streamTurnAudio({ text, signal, onChunk }: AudioStreamOptions): Promise<void> {
  ...
  try {
    ...
    for await (const chunk of audioStream) {
      ...
      // Send chunk with alignment data to callback if provided
      if (onChunk && typedChunk.audioBase64) {
        const alignment = typedChunk.alignment ? {
          characters: typedChunk.alignment.characters,
          characterStartTimesSeconds: typedChunk.alignment.characterStartTimesSeconds,
          characterEndTimesSeconds: typedChunk.alignment.characterEndTimesSeconds,
        } : null;

        const normalizedAlignment = typedChunk.normalizedAlignment ? {
          characters: typedChunk.normalizedAlignment.characters,
          characterStartTimesSeconds: typedChunk.normalizedAlignment.characterStartTimesSeconds,
          characterEndTimesSeconds: typedChunk.normalizedAlignment.characterEndTimesSeconds,
        } : null;

        onChunk(typedChunk.audioBase64, alignment, normalizedAlignment);
      }
    }
